...
import domain.utils.ModelId, domain.DomainImplicits._
...
object QueryAggSqlGen {
  ...
  def modelAggSql(agg: ModelAgg): SqlQueryData = {
    ...
    val fromStr = agg.from.map(from => s" OFFSET ${from - 1} ").getOrElse("")
    val toStr = agg.to.map(to => s" LIMIT $to").getOrElse("")

    (filterStr + fromStr + toStr + ";", filterSet, filterNextIndex, filterUsed)
  }

  def arrayFieldAggSql(agg: ArrayFieldAgg): SqlQueryData = {
    val (filterStr, filterSet, filterNextIndex, filterUsed) = accumulateSqlData(
      "AND",
      1,
      Set.empty,
      filterSql[QueryPredicate](
        _,
        _,
        fieldPredicateSql(agg.parentModel, agg.field.id, _, _)
      ),
      agg.filter
    )
    val fromStr = agg.from.map(from => s" OFFSET ${from - 1} ").getOrElse("")
  ...
  private def accumulateSqlData[A](
    ...
    inputs.foldLeft {
      val initialSqlStr = if (sqlLogicalOp == "AND") "true" else "false"
      (initialSqlStr, HPS.set(()), startParamIndex, usedTables)
    } {
  ...
  private def modelPredicateSql(
    ...
    modelPred.fieldPredicates.foldLeft(
      ("true", HPS.set(()), startParamIndex, Set.empty[UsedTableId])
    ) {
      case ((sql, set, index, used1), (fid, pred)) => {
        val (predSql, predSet, nextStIndex, used2) = fieldPredicateSql(
          modelPred.model.id,
          s"${modelPred.model.id.withQuotes}.${fid.withQuotes}",
          pred,
          index
        )
        (s"$sql AND $predSql", set *> predSet, nextStIndex, used1 ++ used2)
      }
    }
  ...
  private def fieldPredicateSql(
    ...
    pred match {
      case NullPredicate(isNull) =>
        (
          s"$columnId IS${if (isNull) "" else " NOT"} NULL",
          HPS.set(()),
          startParamIndex,
          Set.empty
        )
      ...
      case modelPred @ ModelPredicate(refModel, _) => {
        ...
        val fkEqSql =
          s" AND $columnId = ${refModel.id.withQuotes}.${refModel.primaryField.id.withQuotes} "
        (fieldsStr + fkEqSql, fieldsSet, fieldsNextIndex, usedTables)
      }
      case ArrayPredicate(length) => {
        val arrayTableName = targetModel + "_" + columnId
        val countStr =
          s"(SELECT COUNT(*) FROM ${arrayTableName.withQuotes})"
        val (sql, set, index, _) = andAllSql(startParamIndex) {
          List(
        ...
        (sql, set, index, Set(arrayTableName))
      }
    }
  ...
  private def andAllSql[A: Put](
    ...
    sqlStrs.foldLeft(
      ...
      case ((accSql, set, index, _), Some((sql, value))) =>
        (
          s"$accSql AND $sql",
          set *> HPS.set(index, value),
          index + 1,
          Set.empty
        )
      case (acc, _) => acc
